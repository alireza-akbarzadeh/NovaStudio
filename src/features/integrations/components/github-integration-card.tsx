...
export function GitHubIntegrationCard({
  ...
  const { connection, isConnected, hasRepoScope, isLoading } =
    useGitHubConnection();
      ...
      <div className="flex flex-1 flex-col gap-3 p-5">
        ...
        {isConnected && connection ? (
          <p className="text-[11px] text-muted-foreground">
            ...
            {!hasRepoScope ? " · grant repo access to unlock sync" : null}
          </p>
        ) : null}
        ...
        <div className="mt-auto pt-2">
          <Button
            ...
            disabled={isConnecting || (isConnected && hasRepoScope)}
            onClick={() => void connect()}
          >
            {isConnecting
              ? "Connecting…"
              : isConnected && hasRepoScope
                ? "Connected"
                : isConnected
                  ? "Grant repo access"
                  : "Connect GitHub"}
