...
function mentionHandle(name: string | undefined, email: string | undefined) {
...
function CommentComposer({
  ...
  const memberOptions = useMemo(() => {
    ...
    return (members ?? [])
      .map((m) => ({
        ...
        handle: mentionHandle(m.name, m.email),
        color: m.color,
        imageUrl: m.imageUrl,
      }))
      .filter((m) => {
  ...
  const insertMention = useCallback(
    (handle: string) => {
      ...
      controller.textInput.setInput(`${prefix}@${handle} `);
  ...
  const handleMentionKeyDown = useCallback(
    (event: KeyboardEvent<HTMLTextAreaElement>) => {
      if (!mentionOpen || memberOptions.length === 0) return;
      if (event.key === "Escape") {
      ...
      if (event.key === "ArrowDown") {
        event.preventDefault();
        setMentionIndex((i) => (i + 1) % memberOptions.length);
        return;
      }
      if (event.key === "ArrowUp") {
        event.preventDefault();
        setMentionIndex(
          (i) => (i - 1 + memberOptions.length) % memberOptions.length,
        );
        return;
      }
      if (event.key === "Enter" || event.key === "Tab") {
        const selected =
          memberOptions[
            Math.min(mentionIndex, memberOptions.length - 1)
          ] ?? memberOptions[0];
        if (!selected) return;
        event.preventDefault();
    ...
    [insertMention, memberOptions, mentionIndex, mentionOpen],
  );

  return (
    <div className="relative">
      {mentionOpen && memberOptions.length > 0 ? (
        <div className="absolute right-0 bottom-full left-0 z-10 mb-1 max-h-40 overflow-auto rounded-md border border-ws-border bg-ws-panel p-1 shadow-md">
          <p className="px-2 py-1 text-[10px] text-ws-text-muted">
            Mention a teammate
          </p>
          {memberOptions.map((m, index) => (
            <button
              key={m.userId}
              type="button"
              className={cn(
                ...
                index === mentionIndex
                  ...
                  : "text-ws-text hover:bg-ws-hover",
              )}
              onMouseDown={(event) => {
                event.preventDefault();
            ...
            >
              ...
              <span className="ml-auto shrink-0 text-[10px] text-ws-text-muted">
                @{m.handle}
              </span>
            </button>
          ))}
      ...
      >
        <PromptInputBody>
          <PromptInputTextarea
            ...
            placeholder={
              drafting
                ? "Comment on this line… (@ to mention)"
                : "Reply… (@ to mention)"
            }
        ...
        <PromptInputFooter className="px-2 pb-2">
          <p className="px-1 text-[10px] text-ws-text-muted">
            @mention notifies that teammate
          </p>
        ...
        <ConversationContent className="gap-3 p-3">
          {drafting ? (
            ...
            <ConversationEmptyState
              ...
              description="Right-click the gutter (left of line numbers) → Add comment, or press +."
